...
An extensible and type-safe dependency injection system for Rust, which enables modular architectures with easily composable injectables.
...
This handle is created by leaking a data container to the `'static` lifetime, which makes this library only suitable for applications with a single initialization process during runtime.
