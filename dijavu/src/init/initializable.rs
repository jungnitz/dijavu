use crate::data::DataValue;
use crate::init::InitAppContainer;
use crate::{AppContainerBuilder, Data, InitData, Injectable, Result, RuntimeData};
use dijavu::{DataKey, Error};
use std::convert::Infallible;
use std::marker::PhantomData;
use std::ops::Deref;

/// A value that can be used to configure an [`Injectable`] type
///
/// `Initializable` defines the behavior of a field when using the `Injectable` macro on a struct.
/// In particular, it defines
///
/// - the type of the field in the initialization struct and how the initial value is constructed,
/// - the data required for constructing it during runtime and how this data is constructed from the
///   initialization value, and lastly
/// - how the actual value is constructed from the data available at runtime
///
/// ## Implementations
///
/// dijavu provides some simple, but useful implementations of the trait out of the box.
/// See e.g. [`Value`] or [`StartValue`] for examples.
pub trait Initializable: Sized {
    type Error: Into<Error>;
    /// The type of the data that is modifiable during initialization
    type Init;
    /// The runtime state
    type Runtime;

    /// Creates the initial value for initialization.
    fn new_init_value(container: &mut InitAppContainer) -> Result<Self::Init, Self::Error>;

    /// Consumes the initialization value and constructs the runtime data.
    fn build_runtime_value(
        init: Self::Init,
        data: &mut InitData,
        builder: &mut AppContainerBuilder,
    ) -> Result<Self::Runtime>;

    /// Retrieves the value from the [`RuntimeData`].
    fn from_runtime_value(
        runtime: &'static Self::Runtime,
        data: &RuntimeData,
    ) -> Result<Self, Self::Error>;
}

/// Derive [`Initializable`] for a struct with initializable fields.
///
/// # Example
/// ```
/// # use dijavu::{Value, Initializable};
/// #[derive(Initializable)]
/// struct Test {
///    str: Value<String>,
/// }
/// ```
pub use dijavu_macros::Initializable;

/// A simple, initializable value
///
/// `Value<T>` is a lightweight [`Initializable`] implementation that allows modifying the `T`
/// during initialization and accessing (a reference to) the final value at runtime.
/// The initial value is the default of `T`.
#[derive(Debug)]
pub struct Value<T: 'static>(&'static T);

impl<T> Initializable for Value<T>
where
    T: DataValue + Default,
{
    type Error = Error;
    type Init = T;
    type Runtime = T;

    fn new_init_value(_container: &mut InitAppContainer) -> Result<Self::Init, Self::Error> {
        Ok(T::default())
    }

    fn build_runtime_value(
        init: Self::Init,
        _data: &mut InitData,
        _builder: &mut AppContainerBuilder,
    ) -> Result<T> {
        Ok(init)
    }

    fn from_runtime_value(
        runtime: &'static Self::Runtime,
        _data: &RuntimeData,
    ) -> Result<Self, Self::Error> {
        Ok(Self(runtime))
    }
}

impl<T> Value<T> {
    pub fn into_static_ref(self) -> &'static T {
        self.0
    }
}

impl<T> Deref for Value<T> {
    type Target = T;

    fn deref(&self) -> &Self::Target {
        self.0
    }
}

impl<T> Clone for Value<T> {
    fn clone(&self) -> Self {
        *self
    }
}

impl<T> Copy for Value<T> {}

/// A value that is stored in the start data after initialization
///
/// `StartValue<T>` allows modifying the `T` during initialization.
/// The initial value is the default of `T`.
/// During the build phase, the `T` is then stored in the start data and can be retrieved from it
/// via [`remove_from_start_data`](Self::remove_from_start_data).
///
/// Note that in contrast to some other `Initializable`s, you must only ever initialize a single
/// `StartValue` for any given `T`.
/// Otherwise, the build phase will fail because of duplicate keys in the start data.
/// If you fear that this may happen, wrap your `T` in a local new-type struct to prevent others
/// from using it.
pub struct StartValue<T>(PhantomData<T>);

impl<T> Initializable for StartValue<T>
where
    T: DataValue + Default,
{
    type Error = Error;
    type Init = T;
    type Runtime = ();

    fn new_init_value(_container: &mut InitAppContainer) -> Result<Self::Init, Self::Error> {
        Ok(T::default())
    }

    fn build_runtime_value(
        init: Self::Init,
        _data: &mut InitData,
        builder: &mut AppContainerBuilder,
    ) -> Result<()> {
        builder.insert_start_data::<StartValueKey<T>>(init)?;
        Ok(())
    }

    fn from_runtime_value(
        _runtime: &'static Self::Runtime,
        _data: &RuntimeData,
    ) -> Result<Self, Self::Error> {
        Ok(Self(PhantomData))
    }
}

impl<T: DataValue> StartValue<T> {
    /// Removes the instance of `T` added to the start data by `StartValue<T>`.
    pub fn remove_from_start_data(start_data: &mut Data) -> Option<T> {
        start_data.remove::<StartValueKey<T>>()
    }
}

struct StartValueKey<T>(PhantomData<T>);

impl<T: DataValue> DataKey for StartValueKey<T> {
    type Item = T;
}

impl<T> Clone for StartValue<T> {
    fn clone(&self) -> Self {
        *self
    }
}

impl<T> Copy for StartValue<T> {}

/// A value that is simply dropped on build.
///
/// `StartValue<T>` allows modifying the `T` during initialization.
/// The initial value is the default of `T`.
/// During the build phase, the `T` is then dropped.
///
/// This type is useful when you want to take and use the value of `T` while building the
/// `InitInitializable` that contains this value.
pub struct DropValue<T>(PhantomData<T>);

impl<T> Initializable for DropValue<T>
where
    T: Default,
{
    type Error = Infallible;
    type Init = T;
    type Runtime = ();

    fn new_init_value(_container: &mut InitAppContainer) -> Result<Self::Init, Self::Error> {
        Ok(T::default())
    }

    fn build_runtime_value(
        _init: Self::Init,
        _data: &mut InitData,
        _builder: &mut AppContainerBuilder,
    ) -> Result<Self::Runtime> {
        Ok(())
    }

    fn from_runtime_value(
        _runtime: &'static Self::Runtime,
        _data: &RuntimeData,
    ) -> Result<Self, Self::Error> {
        Ok(Self(PhantomData))
    }
}

impl<T> Clone for DropValue<T> {
    fn clone(&self) -> Self {
        *self
    }
}

impl<T> Copy for DropValue<T> {}

/// Indicates a dependency on an [`Injectable`]
///
/// An instance of this type is automatically provided
///
/// - in the init struct generated by the `Injectable` derive macro for all fields marked with
///   `#[inject]`, and
/// - by the [`Initializable`] implementation of this type, e.g. for use in the `Initializable`
///   derive macro.
///
/// Alternatively, you may obtain an instance by using [`Dependency::new`], but you probably don't
/// want to do that, unless you know what you are doing.
pub struct Dependency<T>(PhantomData<fn() -> T>);

impl<T> Dependency<T>
where
    T: Injectable,
{
    pub fn new(container: &mut InitAppContainer) -> Result<Self, T::Error> {
        T::get_init(container)?;
        Ok(Self(PhantomData))
    }
}

impl<T> Initializable for Dependency<T>
where
    T: Injectable,
{
    type Error = T::Error;
    type Init = Self;
    type Runtime = Dependency<T>;

    fn new_init_value(container: &mut InitAppContainer) -> Result<Self::Init, Self::Error> {
        Self::new(container)
    }

    fn build_runtime_value(
        init: Self::Init,
        _data: &mut InitData,
        _builder: &mut AppContainerBuilder,
    ) -> Result<Self::Runtime> {
        Ok(init)
    }

    fn from_runtime_value(
        _runtime: &'static Self::Runtime,
        _data: &RuntimeData,
    ) -> Result<Self, Self::Error> {
        Ok(Self(PhantomData))
    }
}

impl<T> Initializable for T
where
    T: Copy + Default,
{
    type Error = Infallible;
    type Init = T;
    type Runtime = T;

    fn new_init_value(_container: &mut InitAppContainer) -> Result<Self::Init, Self::Error> {
        Ok(T::default())
    }

    fn build_runtime_value(
        init: Self::Init,
        _data: &mut InitData,
        _builder: &mut AppContainerBuilder,
    ) -> Result<Self::Runtime> {
        Ok(init)
    }

    fn from_runtime_value(
        runtime: &'static Self::Runtime,
        _data: &RuntimeData,
    ) -> Result<Self, Self::Error> {
        Ok(*runtime)
    }
}
